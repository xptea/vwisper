...
fn get_shortcuts_path(app: &AppHandle) -> std::path::PathBuf {
    let local_data = app.path().local_data_dir().expect("Failed to get local data dir");
...
pub async fn save_shortcuts(app: AppHandle, shortcuts: Shortcuts) -> Result<(), String> {
    ...
    let data = serde_json::to_string_pretty(&shortcuts).map_err(|e| format!("Failed to serialize: {}", e))?;
...
pub async fn add_shortcut(app: AppHandle, phrase: String, text: String) -> Result<(), String> {
    ...
    let normalized = normalize(&phrase);
    shortcuts.shortcuts.insert(normalized, ShortcutData { original: phrase, text });
