...
pub struct ProcessTextResult {
    pub shortcut_used: Option<String>, // The shortcut phrase if one was triggered
    pub expanded_text: Option<String>, // The expanded text if shortcut was used
}

#[command]
pub async fn process_text(app: AppHandle, text: String) -> Result<ProcessTextResult, String> {
    ...
    if let Some(data) = shortcuts.shortcuts.get(&normalized) {
        ...
        Ok(ProcessTextResult {
            shortcut_used: Some(data.original.clone()),
            expanded_text: Some(data.text.clone()),
        })
    } else {
